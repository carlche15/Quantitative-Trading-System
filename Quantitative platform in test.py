...
class Operator:
    ...
    def portfolio_update(self,weights=0):
        """
        there ought to be 2 kinds of update function:
        #1 strategic test, in this case the current_stock_price  would just be the initial price
        # real time adjustment strategy, here the  current_stock_price should be the price when the strategy is launched!


        :param weights:
        :return:
        """
        current_capital_temp=self.portfolio.portfolio_value.values[-1]
        for i in range(len(self.portfolio.tickers)):
            current_stock_price=self.portfolio.portfolio["Latest close"].values[i][-1]
            stock_capital_temp=weights[i]*current_capital_temp
            stock_share_temp=round(stock_capital_temp/current_stock_price)
