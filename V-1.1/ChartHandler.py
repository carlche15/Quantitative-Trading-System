...
class Portfolio_ChartHandler():
    ...
    def weightshandler(self):
        description_str1 = "lweights"
        description_str2 = "sweights"
        holdings=self.portfolio.current_holdings[:-1]
        l_index=[]
        l_data=[]
        s_index=[]
        s_data=[]
        for i in holdings.index:
            if holdings[i]>=0:
                l_index.append(i)
                l_data.append(holdings[i])
            else:
                s_index.append(i)
                s_data.append(holdings[i])
        l_holdings=pd.Series(l_data,l_index)
        s_holdings=pd.Series(s_data,s_index)

        return description_str1, l_holdings, description_str2,s_holdings
