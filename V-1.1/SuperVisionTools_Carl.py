...
class Chart_Factory_Portfolio:
  ...
  def plot_weights_l(self,ax):
      ax=ax
      if (len(self.portfolio["lweights"]) != 0):
               index=self.portfolio["lweights"].index
               data=abs(self.portfolio["lweights"].values)
               colors =["limegreen","deepskyblue","plum","darkorange","tomato","y"]
               ax.set_title("Long Positions Summary", fontsize=14)

               ax.axis("equal")
               ax.pie(data,labels=index,colors=colors,shadow=True,autopct='%1.1f%%', startangle=90)
  ...
  def plot_weights_s(self, ax):
          ax = ax
          if (len(self.portfolio["sweights"]) != 0):
              index = self.portfolio["sweights"].index
              data = abs(self.portfolio["sweights"].values)
              colors = ["limegreen", "deepskyblue", "plum", "darkorange", "tomato", "y"]

              ax.set_title("Short Positions Summary", fontsize=14)
  ...
  def plot_all(self):
      ...
      ax=plt.subplot2grid((10,10),(0,0),colspan=10,rowspan=4)
      # ax.set_axis_bgcolor('darkslategray')
      self.plot_portfolio_val(ax)

      if(self.figure_map["pnl"]):
                     self.indicator2+=1
                     ax = plt.subplot2grid((13, 10), (6, 0), colspan=10, rowspan=3)
                     self.plot_pnl(ax)

      if (self.figure_map["weights"]):
                     ax = plt.subplot2grid((13, 10), (10, 0), colspan=3, rowspan=3)
                     self.plot_weights_l(ax)

      if (self.figure_map["weights"]):
                     ax = plt.subplot2grid((13, 10), (10, 5), colspan=3, rowspan=3)
                     self.plot_weights_s(ax)
